...
def getCharmanderLink(soup):
    link_tag = soup.find('a', text='Charmander')
    return 'https://pokemondb.net' + link_tag.get('href')
...
def getEggMoves(pokemon):
    ...
    h3_tag = soup.find('h3', text='Egg moves')
    tbody_tag = h3_tag.findNext('tbody')
    for tr_tag in tbody_tag.find_all('tr'):
        return_list.append(tr_tag.find('td').text)
    return return_list
